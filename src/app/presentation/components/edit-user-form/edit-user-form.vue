      ...
      <div class="pt-4">
        <button
          @click="
            async () => {
              try {
                await editUserController.editPerson(personID!, person);
                router.push('/');
              } catch (error) {
          ...
          "
          type="submit"
          :disabled="editUserController.isBusy"
...
import { PersonFormProps } from "../create-user-form/create-user-form";
...
const editUserController = reactive(new EditUserController(personID.value));

const person = ref<TPerson>(editUserController.user);

onMounted(async () => {
  person.value = await editUserController.getPersonData(personID.value!);
});
</script>
